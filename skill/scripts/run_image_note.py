...
设计原则：**本脚本不含任何提示词正文**。
- 提示词 → `pipeline_loader` 从 `prompts/*.md` 实时装配（改提示词请改 prompts/）
- 素材 → `pipeline_loader` 从 `knowledge_base/` 实时读取
- 动作 → `pipeline_loader` 从 `references/03-action-library.md` 实时抽条（按配额 + 空间适配）

流程：首图 X（环境 + 人脸 + 上身 + 平铺 多图参考）→ 各组以首图为唯一参考生成。
纪律：串行 · 可续跑（已存在即跳过）· 读产物一律 glob · 参考图先缩到长边 ≤1024。

用法：
  python3 run_image_note.py --pkg /tmp/pkg.json --row 2 --outdir ./out/row2 \
      --cover cover.jpg --face face.png --up1 up1.jpg --up2 up2.jpg --flat flat.jpg
  加 --dry 只打印 prompt（不调用模型），用来校对提示词改动。
"""
...
from image_api import generate as api_generate   # noqa: E402
...
def gen(prompt: str, base: str, refs: list[str], tag: str, size: str, model: str) -> dict:
    """生成一张并记录；image_api 内部已重试，这里只负责落盘判定与日志。"""
    t0 = time.time()
    out = api_generate(prompt, refs, model, size, base + ".png")
    dt = time.time() - t0
    ...
    print(f"[{tag}] ❌ 失败 {dt:.0f}s", flush=True)
    return {"tag": tag, "file": None, "ok": False, "sec": round(dt, 1), "dims": None}
...
def main() -> int:
    ...
    ap.add_argument("--cover", required=True, help="参考图1：封面（只借空间与光影）")
    ...
    size = a.size or pkg["facts"].get("size_1k", "1024x1536")
    groups = pkg["groups"]
    ...
    if glob.glob(hero_base + ".*"):
        ...
        manifest.append(gen(hero_prompt, hero_base, refs, f"X{a.row}首图", size, model))
    hero_hits = glob.glob(hero_base + ".*")
    if not hero_hits:
        print("⛔ 首图未生成，组图依赖首图做一致性锚点 —— 先修复首图再续跑（脚本可续跑）", file=sys.stderr)
        return 1
    ...
    for i, gp in enumerate(group_prompts, 1):
        ...
        if glob.glob(base + ".*"):
            ...
            continue
        manifest.append(gen(gp, base, [hero_ref], f"组{i}({col})", size, model))
    ...
    print("下一步：降噪 → 内嵌回 xlsx（保住 DISPIMG）→ 交付闸门")
