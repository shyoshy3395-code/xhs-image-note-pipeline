...
QUOTA = {
    ...
    #      第9组自由补充；看镜头 ≤2；**背面 ≤1**（2026-09-21b 新增）；含 1 组服装特写（正面｜局部特写）
    "9s": [("正面", "全身", "看镜头"), ("3-4侧", "七分", "看向画外"), ("全侧", "全身", "看向画外"),
           ("正面", "半身", "看向画外"), ("背面", "全身", "不可见"), ("3-4侧", "半身", "低头"),
           ("全侧", "七分", "看向画外"), ("正面", "局部特写", "不可见"), ("3-4侧", "全身", "看向画外")],
}
QUOTA_FLOOR = {   # 闸门用：各维度下限（与 04-nine-groups.md 的配额表一致）
    ...
    "9s": {"机位": {"正面": 1, "3-4侧": 1, "全侧": 1, "背面": 1},
           ...
           "组合": {"正面|全身": 1},
           # 上限口径（2026-09-21b）：机位出现次数上限 —— 背面最多 1 组
           "上限": {"背面": 1}},
}
...
def check_quota(actions: list[dict], groups: int) -> list[str]:
    ...
    # 上限口径（如「背面 ≤1」）：机位出现次数不得超过上限
    if floor.get("上限"):
        for k, cap in floor["上限"].items():
            if cams.get(k, 0) > cap:
                bad.append(f"机位 {k} 需 ≤{cap}，实际 {cams.get(k, 0)}")
